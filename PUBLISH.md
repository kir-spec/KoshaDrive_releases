# Публикация: KoshaDrive_releases
...
| | |
|---|---|
| Папка | `E:\coding\KoshaDrive_releases` |
| GitHub | remote `origin` — см. `git remote -v` |
| Cursor Origin | `amalan/KoshaDrive_releases` |
| Remote Cursor | `cursor` → `https://origin.cursor.com/amalan/KoshaDrive_releases.git` |
...
```powershell
wsl -d Ubuntu -e bash -lc 'cd /mnt/e/coding/KoshaDrive_releases && git push -u cursor main'
```

Первичная настройка remote (один раз):

```bash
git remote add cursor https://origin.cursor.com/amalan/KoshaDrive_releases.git
```
...
wsl -d Ubuntu -e bash -lc 'cd /mnt/e/coding/KoshaDrive_releases && git push cursor vДАТА'
